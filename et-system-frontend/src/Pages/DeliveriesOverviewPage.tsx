...
const DeliveriesOverviewPage = () => {
    ...
    const fetchScheduled = async () => {
        const response = await fetch('http://backend:8080/api/deliveries/fetch-unprocessed-planned');
        if (!response.ok){
    ...
    const fetchRecorded = async () => {
        const response = await fetch ('http://backend:8080/api/deliveries/fetch-recorded');
        if (!response.ok){
            throw new Error("response from fetch-planned request was not ok");
        }
        return await response.json();
    ...
    const cancelDelivery = () => {
        fetch(`http://backend:8080/api/deliveries/delete-planned-delivery/${selectedPDelivery}`,{
            method: "POST",
            headers: {
