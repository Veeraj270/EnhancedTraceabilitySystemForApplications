...
const AddRecipePage = () => {
    ...
    const fetchIngredientTypes = async () => {
        const response = await fetch("http://backend:8080/api/ingredients/fetch-ingredient-types")
        if (!response.ok){
    ...
    const handleSubmit = async (event: any) => {
        ...
        event.preventDefault();
        try {
            const response = await fetch('http://backend:8080/api/recipes/add-recipe', {
                method: 'POST',
                headers: {
